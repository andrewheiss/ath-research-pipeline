...
pipeline_base <- airtable("appw9GaXRQ9sCEUtT", c("Papers", "Scores"))
```
...
load_data <- function() {
  ...
  pipeline <- pipeline_base$Papers$select_all(fields = list("Paper", "Status")) %>% 
    mutate(Status = unlist(Status)) %>% 
    select(paper_id = id, Paper, score_id = Status) %>% 
